...
def get_crypto_market_map(crypto):
    ...
    with col1:
        st.write(
            "Exchanges are business that give cusotmers platform to trade cryptocutrences for other digital currencies and even fiat currencies")
        ...
        if len(selected_markets) > 0:
            ...
            market_dataframe = pd.merge(market_dataframe.reset_index(),
                                        ...
                                        left_index=True,
                                        right_index=True)
            data_of_interest = st.radio(label="choose data point to plot on map",
                                        options=['pairPrice', 'price', 'volume'])
